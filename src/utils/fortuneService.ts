...
export const getFortune = async (request: FortuneRequest): Promise<FortuneResponse> => {
  ...
  if (request.question && request.question.trim().length > 0) {
    content += `\n\n关于您提出的问题："${request.question}"，命运之轮显示：${generateSpecificAnswer(request.question, zodiacSign)}`;
  }
...
function generateLoveFortune(nameLength: number, zodiacSign: string): string {
  const fortunes = [
    ...
    `命运之轮显示，您的爱情生活即将迎来重要转折。单身的您可能在一次意外的相遇中遇到心仪对象。已有伴侣的您，关系将更加深入，有望迈向更稳定的阶段。记住，真正的爱情需要双方共同努力和理解。`
  ];
...
function generateCareerFortune(nameLength: number, zodiacSign: string): string {
  const fortunes = [
    ...
    `星象显示您的职业运势正在变得明朗。您过去的努力将开始得到回报，特别是在创新方面的贡献。保持对新知识的渴望，不断学习和提升自己。未来可能有意想不到的职业机会出现，要保持开放的心态和敏锐的洞察力。`
  ];
...
function generateHealthFortune(nameLength: number, zodiacSign: string): string {
  const fortunes = [
    ...
    `星象显示您的身体能量正在恢复。这是增强体质的好时机，可以开始一项新的运动或健身计划。特别注意保护关节和脊椎健康。平衡的饮食、充足的休息和适当的运动将帮助您保持活力和健康。记住，身心健康是相互关联的。`
  ];
...
function generateWealthFortune(nameLength: number, zodiacSign: string): string {
  const fortunes = [
    ...
    `您的财富运势正在上升。可能会有意外的收入或投资机会。然而，星象也提示您要谨慎对待看似诱人的快速致富计划。建立应急基金，减少不必要的债务，并考虑咨询专业财务顾问，这将帮助您实现财务稳健增长。`,
    `命运之轮显示您的财务状况可能面临一些波动。这是审视和调整财务计划的好时机。减少非必要开支，增加储蓄率。未来可能有新的收入来源，但需要您主动寻找和把握机会。记住，财务自由来自于明智的决策和长期规划。`
  ];
...
function generateGeneralFortune(nameLength: number, zodiacSign: string): string {
  const fortunes = [
    ...
    `星象显示您正进入一个自我发现和成长的时期。这是反思过去和规划未来的好时机。您可能会重新评估自己的价值观和目标。保持开放的心态，不要害怕探索新的道路。您的韧性和创造力将帮助您克服障碍，实现真正的自我。`
  ];
...
function generateSpecificAnswer(question: string, zodiacSign: string): string {
  ...
  const answers = [
    `命运之轮显示这条路径充满挑战但值得尝试。保持耐心和毅力，结果将超出您的预期。关键是不要轻易放弃，即使遇到困难也要坚持自己的目标。`,
    `星象预示这可能不是最佳时机。建议再等待一段时间，同时做好充分准备。在未来三个月内，更有利的机会将会出现。`,
    `您的问题背后隐藏着更深层次的考量。建议您反思自己真正的需求和动机。宇宙的能量支持您寻找真实的答案，而不仅仅是表面的解决方案。`,
    `命运指引您向前迈进。虽然道路上有不确定性，但您具备克服挑战的能力。信任自己的判断，同时保持灵活性，调整计划以适应变化的情况。`,
    `星象显示这是一个需要平衡和耐心的时期。在做出决定前，确保您已经考虑了所有可能的选项和后果。咨询您信任的人可能会带来新的视角。`
  ];
